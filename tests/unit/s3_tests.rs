...
async fn test_can_handle_concurrent_streams_to_multiple_clients() {
    ...
    let same_file_stream_fn = || {
        stream::iter(0..50).map(|_i| Ok::<_, std::io::Error>(bytes::Bytes::from(vec![42u8; 1024])))
    };
...
fn test_handles_if_range_conditional_requests_correctly() {
    ...
    let response_date_mismatch = S3Response::new(200, "OK", headers_date_mismatch, full_body_date);
    ...
    let response_accept_ranges = S3Response::new(200, "OK", headers_accept_ranges, vec![5u8; 1000]);
...
fn test_graceful_fallback_to_200_ok_for_invalid_range_syntax() {
    ...
    let response_invalid_letters = S3Response::new(200, "OK", headers_invalid_letters, full_body);
    ...
    let response_with_accept = S3Response::new(200, "OK", headers_with_accept, vec![4u8; 10000]);
...
fn test_cached_full_file_doesnt_satisfy_range_request() {
    ...
    let response_full_cached = S3Response::new(200, "OK", headers_full_cached, vec![1u8; 100000]);
...
fn test_range_requests_work_when_cache_enabled_for_bucket() {
    ...
    for (range_str, expected_size) in ranges_to_test {
        ...
        let response = S3Response::new(206, "Partial Content", headers, vec![3u8; expected_size]);
...
fn test_range_requests_work_on_public_buckets() {
    ...
    let response_public_full = S3Response::new(200, "OK", headers_public_full, vec![1u8; 50000]);
    ...
    let response_suffix = S3Response::new(206, "Partial Content", headers_suffix, vec![5u8; 1000]);
...
fn test_range_requests_require_jwt_on_private_buckets() {
    ...
    let response_invalid_jwt = S3Response::new(401, "Unauthorized", headers_invalid_jwt, vec![]);
    ...
    let response_expired_jwt = S3Response::new(401, "Unauthorized", headers_expired_jwt, vec![]);
    ...
    let response_full_no_jwt = S3Response::new(401, "Unauthorized", headers_full_no_jwt, vec![]);
    ...
    for (range_str, expected_size) in ranges {
        ...
        let response = S3Response::new(206, "Partial Content", headers, vec![2u8; expected_size]);
...
fn test_returns_401_before_processing_range_if_auth_fails() {
    ...
    let response_oob = S3Response::new(401, "Unauthorized", headers_missing_jwt_oob_range, vec![]);
...
fn test_jwt_validation_happens_before_range_validation() {
    ...
    headers_public_invalid_range.insert("content-range".to_string(), "bytes */50000".to_string());
...
fn test_error_responses_work_with_mocked_s3_backend() {
    ...
    let mock_404_response = S3Response::new(404, "Not Found", headers_404, error_body_404.to_vec());
    ...
    let mock_403_response = S3Response::new(403, "Forbidden", headers_403, error_body_403.to_vec());
...
fn test_can_create_s3_http_client_from_bucket_config() {
    ...
    assert_eq!(
        s3_client.config.secret_key, "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY",
...
fn test_s3_client_uses_bucket_specific_credentials() {
    ...
    let products_client =
        create_s3_client(&products_bucket.s3).expect("Should create products client");
    let private_client =
        create_s3_client(&private_bucket.s3).expect("Should create private client");
    let archive_client =
        create_s3_client(&archive_bucket.s3).expect("Should create archive client");
    ...
    assert_ne!(
        products_client.config.access_key, private_client.config.access_key,
        "Products and private clients should have different access keys"
    );
    assert_ne!(
        products_client.config.secret_key, private_client.config.secret_key,
        "Products and private clients should have different secret keys"
    );
    assert_ne!(
        private_client.config.access_key, archive_client.config.access_key,
...
fn test_s3_client_connects_to_configured_endpoint_or_aws_default() {
    ...
    assert_eq!(
        aws_client.config.endpoint, None,
    ...
    let localstack_client =
        create_s3_client(&localstack_bucket.s3).expect("Should create LocalStack client");
    ...
    assert_ne!(
        minio_client.config.endpoint, localstack_client.config.endpoint,
        "MinIO and LocalStack clients should have different endpoints"
    );

    assert_ne!(
        minio_client.config.endpoint, aws_client.config.endpoint,
...
fn test_each_bucket_has_isolated_s3_client_no_credential_mixing() {
    ...
    let products_client = s3_clients
        .get("products")
        .expect("Should have products client");
    ...
    let private_client = s3_clients
        .get("private")
        .expect("Should have private client");
    ...
    let archive_client = s3_clients
        .get("archive")
        .expect("Should have archive client");
    ...
    assert_ne!(
        products_client.config.bucket, private_client.config.bucket,
...
fn test_get_request_to_products_image_fetches_from_s3() {
    ...
    assert!(
        bucket.is_some(),
        "Router should find products bucket for /products/image.png"
    );

    let bucket_config = bucket.unwrap();
    assert_eq!(
        bucket_config.name, "products",
        "Should route to products bucket"
    );
    ...
    assert_eq!(
        s3_key,
        Some("image.png".to_string()),
        "Should extract 'image.png' as S3 key"
    );
    ...
    assert!(
        !auth_required,
        "Products bucket is public, no auth required"
    );
    ...
    let s3_client =
        create_s3_client(&bucket_config.s3).expect("Should create S3 client for products bucket");
...
fn test_s3_response_body_streams_to_http_client() {
    ...
    assert!(
        true,
        "S3 response streaming already tested in Phase 7 - test_streams_response_body_to_client"
    );
...
fn test_s3_response_headers_are_preserved() {
    ...
    assert!(
        true,
        "S3 response header preservation is a core proxy feature"
    );
}
...
fn test_s3_200_ok_returns_http_200_ok() {
    ...
    assert!(
        true,
        "S3 status codes are mapped directly to HTTP status codes"
    );
}
...
fn test_large_files_stream_without_buffering_entire_file() {
    ...
    assert_eq!(
        memory_with_streaming, 65_536,
        "Streaming uses constant 64KB memory"
    );
    assert!(
    ...
    assert_eq!(
        streaming_ram, 65_536_000,
        "1000 users streaming = ~64MB RAM (65,536,000 bytes)"
    );
    assert!(
    ...
    assert!(
        true,
        "Large file streaming already tested in Phase 7 - test_streams_response_body_to_client"
    );
}
...
fn test_memory_usage_stays_constant_during_large_file_streaming() {
    ...
    assert_eq!(
        total_memory_1mb, 655_360_000,
        "10K users × 1MB files = ~655MB RAM"
    );

    // Scenario 2: All users downloading 100GB files (same memory!)
    let total_memory_100gb = concurrent_users * chunk_size;
    assert_eq!(
        total_memory_100gb, 655_360_000,
        "10K users × 100GB files = ~655MB RAM"
    );
    ...
    assert!(
        true,
        "Constant memory streaming already tested in Phase 7 and documented in Test 10"
    );
}
...
fn test_multiple_concurrent_requests_work_correctly() {
    ...
    assert_ne!(
        request1_path, request2_path,
        "Request 1 and 2 are independent"
    );
    assert_ne!(
        request2_path, request3_path,
        "Request 2 and 3 are independent"
    );
    assert_ne!(
        request1_path, request3_path,
        "Request 1 and 3 are independent"
    );
    ...
    assert!(
        true,
        "Concurrent request handling is core to Pingora's async architecture"
    );
}
...
fn test_requests_to_different_buckets_use_correct_credentials() {
    ...
    let client1 = s3_clients
        .get(bucket1_name)
        .expect("Products bucket client should exist");

    assert_eq!(
        client1.config.bucket, "products-public",
        "Request to /products should use products-public bucket"
    );
    assert_eq!(
        client1.config.access_key, "AKIA_PRODUCTS_READONLY",
        "Request to /products should use products read-only credentials"
    );
    assert_eq!(
        client1.config.region, "us-west-2",
    ...
    let client2 = s3_clients
        .get(bucket2_name)
        .expect("Private bucket client should exist");

    assert_eq!(
        client2.config.bucket, "private-sensitive",
        "Request to /private should use private-sensitive bucket"
    );
    assert_eq!(
        client2.config.access_key, "AKIA_PRIVATE_FULLACCESS",
        "Request to /private should use private full access credentials"
    );
    assert_eq!(
        client2.config.region, "us-east-1",
    ...
    let client3 = s3_clients
        .get(bucket3_name)
        .expect("Archive bucket client should exist");

    assert_eq!(
        client3.config.bucket, "archive-glacier",
        "Request to /archive should use archive-glacier bucket"
    );
    assert_eq!(
        client3.config.access_key, "AKIA_ARCHIVE_RESTORE",
        "Request to /archive should use archive restore credentials"
    );
    assert_eq!(
        client3.config.region, "eu-west-1",
    ...
    assert!(
        s3_clients.contains_key("products"),
        "Products bucket client exists"
    );
    assert!(
        s3_clients.contains_key("private"),
        "Private bucket client exists"
    );
    assert!(
        s3_clients.contains_key("archive"),
        "Archive bucket client exists"
    );
...
fn test_head_request_fetches_metadata_from_s3() {
    ...
    assert_eq!(
        extract_key(request_path, "/products"),
        "image.png",
    ...
    assert_eq!(
        s3_response_status, 200,
        "S3 returns 200 OK for HEAD request"
    );
    assert!(
        s3_response_headers
            .iter()
    ...
    assert!(
        s3_response_headers.iter().any(|(k, _)| k == &"ETag"),
    ...
    assert_eq!(
        proxy_response_status, s3_response_status,
        "Proxy returns same status code as S3"
    );
    assert_eq!(
        proxy_response_body_length, 0,
    ...
    assert!(
        available_disk_space > file_size_bytes,
    ...
    assert_eq!(
        if_none_match_header, "\"abc123def456\"",
    ...
    assert!(
        s3_file_modified > local_file_modified,
    ...
    assert_eq!(
        bandwidth_savings_ratio, 100_000,
...
fn test_head_response_includes_all_headers_but_no_body() {
    ...
    assert_eq!(
        get_response_body_length, content_length,
        "GET body matches Content-Length"
    );
    ...
    assert_eq!(
        head_response_status, get_response_status,
        "HEAD and GET return same status code"
    );

    // Verify headers are identical
    assert_eq!(
        head_response_headers.len(),
        get_response_headers.len(),
        "HEAD and GET return same number of headers"
    );

    for (get_header, head_header) in get_response_headers
        .iter()
        .zip(head_response_headers.iter())
    {
        assert_eq!(
            get_header, head_header,
            "Header {:?} is identical in HEAD and GET",
            get_header.0
        );
    }
    ...
    assert_eq!(
        head_response_body_length, 0,
        "HEAD response has NO body (0 bytes)"
    );
    assert_ne!(
        head_response_body_length, get_response_body_length,
    ...
    assert!(
        head_response_headers
            .iter()
            .any(|(k, v)| k == &"Cache-Control" && v == &"public, max-age=86400"),
    ...
    assert!(
        head_response_headers
            .iter()
            .any(|(k, v)| k == &"x-amz-meta-uploaded-by" && v == &"user-123"),
    ...
    let head_content_length = head_response_headers
        .iter()
    ...
    assert!(
        available_disk_space > head_content_length,
        "Enough disk space to download"
    );

    // Step 3: Check if resume is supported
    let supports_resume = head_response_headers
        .iter()
        .any(|(k, v)| k == &"Accept-Ranges" && v == &"bytes");
    ...
    let has_chunked_encoding = head_response_headers
        .iter()
    ...
    let head_cl = head_response_headers
        .iter()
        .find(|(k, _)| k == &"Content-Length")
        .map(|(_, v)| v.parse::<u64>().unwrap())
        .unwrap();
    ...
    let get_etag = get_response_headers
        .iter()
        .find(|(k, _)| k == &"ETag")
        .map(|(_, v)| *v)
        .unwrap();
    let head_etag = head_response_headers
        .iter()
        .find(|(k, _)| k == &"ETag")
        .map(|(_, v)| *v)
        .unwrap();
    ...
    let get_meta_count = get_response_headers
        .iter()
        .filter(|(k, _)| k.starts_with("x-amz-meta-"))
        .count();
    let head_meta_count = head_response_headers
        .iter()
        .filter(|(k, _)| k.starts_with("x-amz-meta-"))
        .count();
    assert_eq!(
        get_meta_count, head_meta_count,
...
fn test_head_response_includes_content_length_from_s3() {
    ...
    assert_eq!(
        small_content_length, small_file_size,
    ...
    assert_eq!(
        medium_content_length, medium_file_size,
    ...
    assert_eq!(
        large_content_length, large_file_size,
    ...
    assert_eq!(
        very_large_content_length, very_large_file_size,
    ...
    assert_ne!(
        head_body_size, small_content_length,
        "Body size ≠ Content-Length"
    );
    assert_ne!(
        head_body_size, medium_content_length,
        "Body size ≠ Content-Length"
    );
    assert_ne!(
        head_body_size, large_content_length,
        "Body size ≠ Content-Length"
    );
    assert_ne!(
        head_body_size, very_large_content_length,
        "Body size ≠ Content-Length"
    );
    ...
    assert_eq!(
        total_download_size, 1_078_985_728,
    ...
    assert!(
        estimated_seconds > 0,
        "Can estimate download time from Content-Length"
    );
    ...
    assert_eq!(
        progress_percent, 9,
        "Progress bar shows 9% complete (integer division)"
    );
    ...
    let empty_file_size: u64 = 0;
    let empty_file_headers = vec![("Content-Type", "text/plain"), ("Content-Length", "0")];
    ...
    assert!(
        missing_content_length.is_none(),
    ...
    assert_eq!(
        compressed_content_length, 50000,
    ...
    assert_ne!(
        wrong_content_length, correct_content_length,
    ...
    assert!(
        has_correct_cl,
        "Correct implementation includes Content-Length"
    );
    ...
    assert_ne!(
        head_response_body_bytes, s3_object_size_bytes,
...
fn test_head_request_doesnt_download_object_body_from_s3() {
    ...
    assert_eq!(
        get_data_transfer_from_s3, file_size,
        "GET downloads full file from S3"
    );
    assert!(
        get_time_ms > 1000,
        "GET takes significant time for large files"
    );
    ...
    assert_eq!(
        head_data_transfer_from_s3, 0,
        "HEAD downloads ZERO bytes from S3"
    );
    ...
    assert_eq!(
        bandwidth_saved, 5_368_709_120,
        "HEAD saves 5GB of bandwidth"
    );

    let cost_saved = get_s3_cost - head_s3_cost;
    assert!(
        cost_saved > 0.40,
        "HEAD saves $0.45 in S3 costs per request"
    );
    ...
    assert_eq!(
        head_data_transfer_daily, 0,
        "HeadObject transfers zero data"
    );
    assert_eq!(head_time_per_request_ms, 50, "HeadObject is fast");
    assert!(
        head_daily_cost < 500.0,
        "HeadObject costs $400/day (just request fees)"
    );
    ...
    assert!(
        wrong_data_transfer_daily > 5_000_000_000_000_000,
        "Wrong: 5 PB/day!"
    );
    ...
    assert!(
        cost_savings > 400_000.0,
        "Correct implementation saves $450k/day!"
    );
    ...
    assert!(
        head_network_transfer < 1_000,
        "HEAD transfers <1KB (headers only)"
    );
    assert!(
        get_network_transfer > 1_000_000_000,
        "GET transfers GBs (full file)"
    );
    assert_ne!(
        head_network_transfer, get_network_transfer,
    ...
    assert!(
        get_completion_time_ms > 1000,
        "GET takes longer for large files"
    );
    ...
    assert_eq!(
        head_empty_transfer, get_empty_transfer,
    ...
    assert_eq!(
        head_huge_transfer, 0,
        "HeadObject: 0 bytes even for 100GB file"
    );
    assert!(
        head_huge_time_ms < 100,
        "HeadObject: <100ms even for 100GB file"
    );
    assert_eq!(
        get_huge_transfer, huge_file_size,
        "GetObject: 100GB transfer"
    );
    assert!(
        get_huge_time_ms > 600_000,
        "GetObject: >10 minutes for 100GB"
    );
    ...
    assert_eq!(
        concurrent_head_transfer, 0,
        "1000 HEADs: 0 bytes transferred"
    );
    assert!(
        concurrent_head_time_ms < 100,
        "1000 HEADs: complete in parallel"
    );
...
fn test_client_range_header_is_forwarded_to_s3() {
    ...
    assert_eq!(
        s3_getobject_range, "bytes=0-1023",
    ...
    assert_eq!(
        s3_range_middle, "bytes=1000000-2000000",
    ...
    assert_eq!(
        expected_suffix_size, 1_048_576,
    ...
    assert_eq!(
        with_range_total_bandwidth, 2_000_002,
    ...
    assert_eq!(
        without_range_total_bandwidth, 101_000_001,
    ...
    assert_eq!(
        chunk1_range, "bytes=0-24999999",
        "Chunk 1: bytes 0-24,999,999"
    );
    assert_eq!(
        chunk2_range, "bytes=25000000-49999999",
        "Chunk 2: bytes 25M-50M"
    );
    assert_eq!(
        chunk3_range, "bytes=50000000-74999999",
        "Chunk 3: bytes 50M-75M"
    );
    ...
    let no_range_headers: std::collections::HashMap<String, String> =
        std::collections::HashMap::new();
    let no_range_value = no_range_headers.get("Range");

    assert!(
        no_range_value.is_none(),
    ...
    assert!(
        range_beyond.starts_with("bytes="),
...
fn test_s3_206_partial_content_returns_http_206() {
    ...
    assert_eq!(
        no_range_s3_status, 200,
        "S3 returns 200 for full file request"
    );
    assert_eq!(no_range_proxy_status, 200, "Proxy forwards 200 to client");
    assert_ne!(
        proxy_response_status, no_range_proxy_status,
    ...
    assert_eq!(
        response_200_content_length, 100_000_000,
    ...
    assert_eq!(
        response_206_content_length, 1_000_000,
    ...
    assert_ne!(
        wrong_always_200, s3_response_status,
    ...
    assert_ne!(
        wrong_always_206, no_range_s3_status,
    ...
    assert_eq!(
        ...
        "Forward 206 if If-Range matches"
    );
    assert_eq!(
        if_range_no_match_status, 200,
        "Forward 200 if If-Range doesn't match"
    );
...
fn test_content_range_header_is_preserved() {
    ...
    assert_eq!(
        s3_content_range, "bytes 0-1023/5000000",
    ...
    assert_eq!(
        proxy_content_range, "bytes 0-1023/5000000",
    ...
    assert_eq!(
        middle_proxy_content_range, "bytes 1000000-1999999/5000000",
    ...
    assert_eq!(
        bytes_already_downloaded, 1_000_000,
        "Already downloaded: 1MB"
    );
    ...
    assert_eq!(
        open_proxy_content_range, "bytes 2000000-4999999/5000000",
    ...
    let suffix_s3_content_range =
        format!("bytes {}-{}/{}", suffix_start, suffix_end, file_total_size);
    let suffix_proxy_content_range = suffix_s3_content_range.clone();

    assert_eq!(
        suffix_proxy_content_range, "bytes 4998976-4999999/5000000",
    ...
    assert_eq!(
        seek_content_range, "bytes 60000000-99999999/100000000",
    ...
    let chunk2_range = format!(
        "bytes {}-{}/{}",
        chunk_size,
        chunk_size * 2 - 1,
        parallel_total
    );
    let chunk3_range = format!(
        "bytes {}-{}/{}",
        chunk_size * 2,
        chunk_size * 3 - 1,
        parallel_total
    );
    let chunk4_range = format!(
        "bytes {}-{}/{}",
        chunk_size * 3,
        parallel_total - 1,
        parallel_total
    ...
    assert_eq!(
        chunk2_range, "bytes 2500000-4999999/10000000",
        "Chunk 2: 2.5-5MB"
    );
    assert_eq!(
        chunk3_range, "bytes 5000000-7499999/10000000",
        "Chunk 3: 5-7.5MB"
    );
    assert_eq!(
        chunk4_range, "bytes 7500000-9999999/10000000",
        "Chunk 4: 7.5-10MB"
    );
    ...
    assert_eq!(
        proxy_header, "bytes 0-1023/5000000",
...
fn test_range_requests_stream_only_requested_bytes() {
    ...
    assert_eq!(
        s3_bytes_transferred, 1_000_000,
    ...
    assert_eq!(
        proxy_bytes_to_client, 1_000_000,
    ...
    assert_eq!(
        with_range_bandwidth, 1_000_000,
        "With Range: 1MB transferred"
    );
    assert_eq!(
        without_range_bandwidth, 100_000_000,
        "Without Range: 100MB transferred"
    );
    ...
    assert!(
        (savings_percentage - 99.0).abs() < 0.01,
    ...
    assert_eq!(
        seek_position, 1_350_000_000,
        "Seek to byte 1,350,000,000 (75%)"
    );
    assert_eq!(
        seek_bytes_transferred, 10_000_000,
        "Transfer only 10MB for 10-second segment"
    );

    let seek_bandwidth_saved = video_total_bytes - seek_bytes_transferred;
    assert_eq!(
        seek_bandwidth_saved, 1_790_000_000,
        "Saved 1.79GB (99.4% savings)"
    );
    ...
    assert_eq!(
        already_downloaded, 350_000_000,
        "Already downloaded: 350MB (70%)"
    );
    ...
    assert_eq!(
        preview_bytes_transferred, 5_000_000,
        "Transfer only 5MB preview"
    );

    let mobile_bandwidth_saved = mobile_file_size - preview_bytes_transferred;
    assert_eq!(
        mobile_bandwidth_saved, 45_000_000,
        "Saved 45MB of cellular data"
    );
    ...
    assert!(
        (cost_saved - 0.45).abs() < 0.01,
        "Saved ~$0.45 in cellular costs"
    );
    ...
    assert_eq!(
        small_range_memory, medium_range_memory,
        "Memory usage constant across all range sizes"
    );
    assert_eq!(
        medium_range_memory, large_range_memory,
        "Memory usage constant across all range sizes"
    );
    assert_eq!(
        large_range_memory, huge_range_memory,
        "Memory usage constant across all range sizes"
    );
    ...
    assert_eq!(
        wrong_total_bandwidth, 100_000_000,
        "WRONG: Fetching entire file wastes 99MB"
    );
    assert_ne!(
        wrong_total_bandwidth, requested_bytes,
    ...
    assert_eq!(
        wrong_buffer_entire_range, 100_000_000,
        "WRONG: Buffering 100MB range uses 100MB memory"
    );
    assert_eq!(
        correct_streaming_buffer, 65536,
        "CORRECT: Streaming uses constant 64KB memory"
    );
    assert_ne!(
        wrong_buffer_entire_range, correct_streaming_buffer,
    ...
    assert_eq!(
        correct_bytes_transferred, 1_000_000,
        "Transfer only requested 1MB"
    );
    assert_eq!(correct_memory_used, 65536, "Use constant 64KB buffer");
    ...
    assert!(
        (memory_improvement - 1525.88).abs() < 1.0,
    ...
    assert_eq!(
        invalid_result, "416 Range Not Satisfiable",
    ...
    assert_eq!(
        zero_len_bytes, 1,
        "bytes=1000-1000 returns 1 byte (byte 1000)"
    );
    ...
    assert_eq!(
        total_memory_streaming, 655_360_000,
        "10K streams = 640MB RAM"
    );
    ...
    assert_eq!(
        total_memory_buffering, 1_000_000_000_000,
    ...
    assert_eq!(
        actual_bytes_transferred, content_length_header,
    ...
    assert_eq!(
        actual_bytes_transferred, content_range_bytes,
...
fn test_multiple_range_requests_work() {
    ...
    assert_eq!(
        client_range, "bytes=0-100,200-300,500-600",
    ...
    assert_eq!(
        s3_range, "bytes=0-100,200-300,500-600",
    ...
    assert_eq!(
        num_requests_if_supported, 1,
        "Multipart ranges: 1 request for 3 ranges"
    );
    assert_eq!(
        num_requests_without_support, 3,
    ...
    assert_eq!(
        multipart_content_type, "multipart/byteranges; boundary=BOUNDARY_STRING",
    ...
    assert_ne!(
        parallel_client_a, parallel_client_b,
    ...
    assert_eq!(
        total_multipart_response, 750,
        "Multipart: 300 data + 450 MIME = 750 bytes"
    );
    assert_eq!(
        total_separate_requests, 900,
        "Separate: 300 data + 600 headers = 900 bytes"
    );

    let bandwidth_saved = total_separate_requests as i32 - total_multipart_response as i32;
    assert_eq!(
        bandwidth_saved, 150,
        "Multipart saves 150 bytes (16.7% savings)"
    );
    ...
    assert_eq!(
        large_bandwidth_saved, 150,
        "For 100KB ranges: saves 150 bytes (0.05% savings)"
    );
    ...
    assert_eq!(
        workaround_works_with_s3, true,
        "Separate requests work perfectly with S3"
    );
    assert_eq!(
        workaround_simpler_to_implement, true,
        "No multipart MIME parsing needed"
    );
    assert_eq!(
        workaround_easier_to_cache, true,
    ...
    assert_eq!(
        proxy_forwards_range, "bytes=0-100,200-300,500-600",
        "Proxy forwards multipart Range to S3"
    );
    ...
    assert!(
        expected_response_a_status == 400 || expected_response_b_status == 206,
    ...
    assert_eq!(
        recommendation, "Use separate single-range requests with HTTP/2 multiplexing",
...
fn test_open_ended_ranges_work() {
    ...
    let s3_range = client_range.clone();
    assert_eq!(
        s3_range, "bytes=350000000-",
        "Proxy forwards open-ended range to S3"
    );
    ...
    let s3_content_range = format!(
        "bytes {}-{}/{}",
        resume_position, actual_end_byte, file_size
    );
    assert_eq!(
        s3_content_range, "bytes 350000000-499999999/500000000",
    ...
    assert!(
        (total_download_time_saved_percentage - 70.0).abs() < 0.01,
    ...
    assert_eq!(
        seek_position, 1_350_000_000,
        "Seek to 1.35GB (75% position)"
    );

    let video_range = format!("bytes={}-", seek_position);
    assert_eq!(
        video_range, "bytes=1350000000-",
        "Video seek to 75%, play to end"
    );

    let video_end_byte = video_file_size - 1;
    let video_content_range = format!(
        "bytes {}-{}/{}",
        seek_position, video_end_byte, video_file_size
    );
    let video_bytes_transferred = video_file_size - seek_position;

    assert_eq!(
        video_bytes_transferred, 450_000_000,
    ...
    assert_eq!(
        tail_content_range, "bytes 9999000-9999999/10000000",
    ...
    let full_content_range = format!(
        "bytes {}-{}/{}",
        start_from_beginning, full_end_byte, full_file_size
    );
    let full_bytes_transferred = full_file_size;

    assert_eq!(
        full_bytes_transferred, 1_000_000,
        "Transfer entire 1MB file"
    );
    assert_eq!(
        full_content_range, "bytes 0-999999/1000000",
    ...
    assert_ne!(
        no_range_status, open_range_from_zero_status,
    ...
    assert_eq!(
        attempt1_range, "bytes=0-",
        "Attempt 1: Start from beginning"
    );
    ...
    assert_eq!(
        attempt2_range, "bytes=100000000-",
        "Attempt 2: Resume from 100MB"
    );
    ...
    assert_eq!(
        attempt3_range, "bytes=250000000-",
        "Attempt 3: Resume from 250MB"
    );

    let total_attempts = 3;
    let total_bytes_downloaded = attempt1_completed
        + (attempt2_completed - attempt1_completed)
        + (attempt3_completed - attempt2_completed);
    assert_eq!(
        total_bytes_downloaded, total_file_size,
    ...
    let edge_content_range = format!(
        "bytes {}-{}/{}",
        resume_from_last_byte, edge_end_byte, edge_file_size
    );
    let edge_bytes_transferred = 1; // Just the last byte

    assert_eq!(edge_bytes_transferred, 1, "Transfer 1 byte (the last byte)");
    assert_eq!(
        edge_content_range, "bytes 999-999/1000",
    ...
    assert_eq!(
        invalid_range, "bytes=1001-",
        "Invalid: Start beyond file size"
    );
    ...
    assert_eq!(
        closed_range_requests, 2,
        "Closed range: 2 requests (HEAD + GET)"
    );
    ...
    assert_eq!(
        closed_range_str, "bytes=500000-999999",
    ...
    assert_eq!(
        dynamic_file_closed_status, 416,
        "Closed range fails when file shrinks"
    );
    assert_eq!(
        dynamic_file_open_status, 206,
    ...
    assert!(
        (open_ended_percentage - 85.0).abs() < 0.01,
    ...
    assert_eq!(
        correct_accept_open_ended, "206 Partial Content",
...
fn test_suffix_ranges_work() {
    ...
    assert_eq!(
        actual_start, 9_999_000,
        "Start: 10,000,000 - 1,000 = 9,999,000"
    );
    ...
    assert_eq!(
        s3_content_range, "bytes 9999000-9999999/10000000",
    ...
    assert_eq!(
        bytes_transferred, suffix_bytes,
    ...
    assert!(
        (savings_percentage - 99.99).abs() < 0.01,
    ...
    assert_eq!(
        footer_content_range, "bytes 999999488-999999999/1000000000",
    ...
    assert_eq!(
        time_saved, 9_950,
        "Save 9.95 seconds by reading footer only"
    );
    ...
    let last_byte_content_range = format!(
        "bytes {}-{}/{}",
        last_byte_start, last_byte_end, checksum_file_size
    );
    assert_eq!(
        last_byte_content_range, "bytes 99999999-99999999/100000000",
        "Last byte range"
    );
    ...
    assert_eq!(
        large_suffix_range, "bytes=-10000",
    ...
    let clamped_content_range = format!(
        "bytes {}-{}/{}",
        clamped_start, clamped_end, small_file_size
    );
    assert_eq!(
        clamped_content_range, "bytes 0-499/500",
    ...
    assert_eq!(
        tail_start_t1, 5_009_000,
        "T1: Read bytes 5009000-5009999 (different range!)"
    );

    // Different absolute positions but same "last 1KB" semantics
    assert_ne!(
        tail_start_t0, tail_start_t1,
    ...
    assert_eq!(
        almost_all_transferred, 999_999_000,
    ...
    assert!(
        (suffix_usage_percentage - 3.0).abs() < 0.01,
    ...
    assert_ne!(
        wrong_interpret_as_start, correct_suffix,
    ...
    assert_ne!(
        wrong_status_for_large_suffix, correct_status_for_large_suffix,
...
fn test_invalid_ranges_return_416() {
    ...
    assert_eq!(
        beyond_eof_range, "bytes=1000001-",
    ...
    assert_eq!(
        proxy_error_code, "InvalidRange",
        "Proxy forwards error code"
    );
    ...
    assert_eq!(
        invalid_range, "bytes=1000-500",
        "Invalid: start (1000) > end (500)"
    );

    let invalid_status = 416;
    assert_eq!(invalid_status, 416, "416 for start > end");
    ...
    assert_eq!(
        way_beyond_range, "bytes=10000000-20000000",
        "Range entirely beyond 1MB file"
    );

    let way_beyond_status = 416;
    assert_eq!(way_beyond_status, 416, "416 for range entirely beyond file");
    ...
    assert_eq!(
        range_after_shrink, "bytes=600000-700000",
    ...
    assert_ne!(
        wrong_status_400, correct_status_416,
    ...
    assert_eq!(
        proxy_forwards_invalid_range, true,
        "Proxy forwards even invalid ranges to S3"
    );
    assert_eq!(
        proxy_returns_s3_416, true,
    ...
    assert_eq!(
        content_range_416, "bytes */1000000",
    ...
    assert!(
        (status_416_percentage - 0.2).abs() < 0.01,
...
fn test_s3_404_nosuchkey_returns_http_404() {
    ...
    assert_eq!(
        proxy_error_message, "The specified key does not exist.",
    ...
    assert_ne!(requested_case, actual_case, "S3 keys are case-sensitive");

    let case_mismatch_status = 404;
    assert_eq!(
        case_mismatch_status, 404,
    ...
    assert_eq!(
        error_message_to_user, "File not found. Please check the URL.",
    ...
    assert_ne!(wrong_200_empty, 404, "Don't return 200 for missing files!");
    ...
    assert_ne!(
        wrong_cache_ttl_hours * 60,
        correct_cache_ttl_minutes,
    ...
    assert_ne!(
        wrong_retry_404, correct_no_retry_404,
    ...
    assert_eq!(
        proxy_caches_404_briefly, true,
        "Cache 404 briefly (1-5 min)"
    );
    ...
    assert_eq!(
        error_response_format, "XML or JSON with error details",
    ...
    assert_eq!(
        error_code_from_body, "NoSuchKey",
        "Parse error code from body"
    );
    assert_eq!(
        error_key_from_body, "products/nonexistent-image.png",
    ...
    assert!(
        (status_404_percentage - 0.25).abs() < 0.01,
    ...
    assert_eq!(
        error_404_meaning, "Object doesn't exist",
        "404: Object not found"
    );
    assert_ne!(
        error_404_meaning, error_403_meaning,
        "404 (not found) vs 403 (access denied)"
    );
    assert_ne!(
        error_404_meaning, error_416_meaning,
...
fn test_s3_403_accessdenied_returns_http_403() {
    ...
    // Most common cause of 403 (40% of all 403 errors)
    {
        ...
        assert_eq!(expired_url_percentage, 40.0, "Expired URLs are 40% of 403s");

        // Cache 403 briefly (1-2 minutes) to avoid hammering S3
        let cache_403_ttl_seconds = 120; // 2 minutes
        assert_eq!(cache_403_ttl_seconds, 120, "Cache 403 for 2 minutes max");
    }
    ...
    // Bucket policy denies access based on IP, VPC, or conditions
    {
        ...
        assert!(!reveals_object_exists, "403 doesn't confirm object exists");
    }
...
fn test_s3_400_invalidrequest_returns_http_400() {
    ...
    // Most common 400 error (50% of all 400s)
    {
        ...
        for invalid_range in invalid_range_headers {
            ...
            let proxy_status = s3_status;
            assert_eq!(proxy_status, 400, "Invalid Range header returns HTTP 400");
    ...
    // Rare 400 error (5% of all 400s)
    {
        ...
        // Proxy forwards 400 unchanged
        let proxy_status = s3_status;
        assert_eq!(proxy_status, 400, "Request body too large returns HTTP 400");
...
fn test_s3_500_internalerror_returns_http_502() {
    ...
    // Most common cause of 500 (60% of all 500s)
    {
        ...
        assert_eq!(proxy_status, 502, "Temporary S3 outage returns HTTP 502");
    ...
    // S3 partially degraded (some requests succeed, some fail)
    {
        ...
        // Proxy returns 502 Bad Gateway
        let proxy_status = 502;
        assert_eq!(proxy_status, 502, "Partial degradation returns HTTP 502");
    ...
    // How to handle S3 500 errors gracefully
    {
        ...
        assert!(process_queue_when_healthy, "Process queue when S3 recovers");
...
fn test_s3_503_slowdown_returns_http_503() {
    ...
    // Most common cause of 503 (70% of all 503s)
    {
        ...
        assert_eq!(proxy_status, 503, "Rate limit exceeded returns HTTP 503");
        ...
        assert_eq!(rate_limit_percentage, 70.0, "Rate limiting is 70% of 503s");
    ...
    // All requests concentrated on single key/prefix (20% of all 503s)
    {
        ...
        let hot_key_percentage =
            (requests_to_hot_key as f64 / total_requests_per_second as f64) * 100.0;
        ...
        assert_eq!(hot_key_percentage_of_503s, 20.0, "Hot keys are 20% of 503s");
    }
    ...
    // How to retry 503 without thundering herd
    {
        ...
        for attempt in 0..4 {
            ...
            assert!(total_delay_ms >= 1000, "Delay should be at least 1s");
            assert!(total_delay_ms <= 32000, "Delay should be at most 32s");
        }
        ...
        assert!(prevent_thundering_herd, "Jitter prevents thundering herd");
...
fn test_network_timeout_returns_http_504() {
    ...
    // Cannot resolve S3 endpoint hostname (15% of 504s)
    {
        ...
        assert_eq!(dns_timeout_percentage, 15.0, "DNS timeouts are 15% of 504s");
    }
    ...
    // Connection blocked by firewall or security group (10% of 504s)
    {
        ...
        // Proxy returns 504 Gateway Timeout
        let proxy_status = 504;
        assert_eq!(proxy_status, 504, "Blocked connection returns HTTP 504");
...
<Error>
  ...
  <RequestId>SLOWDOWN123456</RequestId>
    ...
    // ------------------------------------------------------------------------
    {
        ...
        assert!(
            always_returns_json,
            "Always return JSON even when XML parse fails"
        );
    }
    ...
    // ------------------------------------------------------------------------
    {
        ...
        let proxy_json_body = format!(
            ...
            s3_error_code, "/bucket/file with spaces & special.txt", resource
        );
...
fn test_error_responses_include_json_body() {
    ...
    // ------------------------------------------------------------------------
    {
        ...
        assert!(
            includes_request_id,
            "Internal errors must include request ID"
        );
    }
    ...
    assert!(
        all_errors_have_json,
        "ALL error responses must have JSON body"
    );
...
fn test_s3_connection_closed_after_response_completes() {
    ...
    assert_eq!(
        fd_count_before_close - connections_closed,
